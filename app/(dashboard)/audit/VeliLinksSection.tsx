...
export default function VeliLinksSection({
  ...
  const kapali = tokens.filter(t =>  revokedJtis.has(t.jti))

  const shown = tab === 'aktif' ? aktif : tab === 'gecmis' ? gecmis : kapali

  function handleRevoke(token: VeliToken) {
    setRevokingId(token.id)
    setError(null)
    startTransition(async () => {
      const result = await revokeToken(token.jti, 'veli', 'Denetim sayfasından kapatıldı')
      if (result.ok) {
  ...
  const tabs: { key: Tab; label: string; count: number }[] = [
    { key: 'aktif',  label: 'Aktif Linkler',     count: aktif.length  },
    { key: 'gecmis', label: 'Süresi Geçmişler',   count: gecmis.length },
    { key: 'kapali', label: 'Kapatılanlar',        count: kapali.length },
  ]
  ...
  return (
    <section className="bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-2xl overflow-hidden">
      <div className="px-5 pt-5 pb-3">
        <h2 className="text-base font-bold text-gray-900 dark:text-slate-100">Veli Erişim Linkleri</h2>
        <p className="text-xs text-gray-500 dark:text-slate-400 mt-0.5">
          Öğrenci sayfalarından oluşturulan veli linkleri · Aktif linkler 7 gün geçerli
        </p>
      </div>
      ...
      {shown.length === 0 ? (
        <p className="px-5 py-8 text-center text-sm text-gray-400">
          {tab === 'aktif'  ? 'Geçerli veli linki yok.' :
           tab === 'gecmis' ? 'Süresi geçmiş link yok.' :
                              'Kapatılmış link yok.'}
        </p>
      ) : (
          ...
          {shown.map(token => {
            ...
            return (
              <div key={token.id} className="flex items-center gap-3 px-5 py-3">
                {/* Sol: öğrenci + kim oluşturdu */}
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium text-gray-900 dark:text-slate-100 truncate">
                    {token.students?.full_name ?? 'Bilinmeyen Öğrenci'}
                  </p>
                  <p className="text-xs text-gray-500 dark:text-slate-400 mt-0.5">
                    {profileMap.get(token.issued_by) ?? 'Bilinmeyen'} oluşturdu ·{' '}
                  ...
                  {revoked ? (
                    ...
                    <div className="flex items-center gap-2 justify-end">
                      ...
                      <button
                        ...
                        className="text-[11px] font-semibold px-2.5 py-1 rounded-full bg-red-50 text-red-600 hover:bg-red-100 dark:bg-red-950/40 dark:text-red-400 dark:hover:bg-red-950/70 transition-colors disabled:opacity-50"
                      >
                        {isPending && revokingId === token.id ? '…' : 'Erişimi Kapat'}
                      </button>
                    </div>
                  )}
