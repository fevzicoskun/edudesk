  ...
  return (
    <div className="space-y-6">
      ...
      <section>
        ...
        <ScheduleList items={resmi} getName={getName} getSubtitle={getSubtitle} deletingId={deletingId} onDelete={handleDelete} />
        {resmi.length === 0 && <EmptyState label="Resmi Ders Programı" />}
      </section>
      ...
      <section>
        <h2 className="text-sm font-semibold text-gray-700 dark:text-slate-300 mb-3 flex items-center gap-2">
          ...
          {editingLabel ? (
            ...
            <span className="flex items-center gap-1.5">
              {labelValue}
              <button onClick={() => setEditingLabel(true)} title="İsmi değiştir"
        ...
        <ScheduleList items={okul} getName={getName} getSubtitle={getSubtitle} deletingId={deletingId} onDelete={handleDelete} />
        {okul.length === 0 && <EmptyState label={labelValue} />}
      </section>
    </div>
  )
}
...
function EmptyState({ label }: { label: string }) {
  return (
    <p className="text-sm text-gray-400 dark:text-slate-500 py-4 text-center border border-dashed border-gray-200 dark:border-slate-700 rounded-xl">
      Henüz {label} eklenmemiş.
    </p>
  )
}

function ScheduleList({ items, getName, getSubtitle, deletingId, onDelete }: {
  items: Schedule[]
        ...
        <li key={s.id} className="flex items-center justify-between gap-3 p-3 bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-xl">
          <div className="min-w-0 flex-1">
            ...
            <p className="text-xs text-gray-400 dark:text-slate-500 mt-0.5">
              {s.file_name
                ? <span className="text-emerald-600 dark:text-emerald-400">✓ {s.file_name}</span>
                : <span className="text-amber-500">Dosya yüklenmemiş</span>}
            </p>
          </div>
          <div className="flex items-center gap-2 shrink-0">
            <Link href={`/ders-programi/${s.id}`}
