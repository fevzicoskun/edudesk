...
function dayLabel(days: number | null): { text: string; cls: string } {
  if (days === null) return { text: 'Hiç kullanılmadı', cls: 'text-gray-400 dark:text-slate-500' }
  if (days === 0)    return { text: 'Bugün kullanıldı',   cls: 'text-emerald-600 dark:text-emerald-400' }
  if (days <= 7)     return { text: `${days} gün önce`,   cls: 'text-emerald-600 dark:text-emerald-400' }
  if (days <= 14)    return { text: `${days} gün önce`,   cls: 'text-amber-600 dark:text-amber-400' }
  return               { text: `${days} gün önce`,        cls: 'text-red-600 dark:text-red-400' }
}

function urgencyBg(days: number | null): string {
  if (days === null || days > 14) return 'bg-red-50 dark:bg-red-950/30 border-red-200 dark:border-red-800'
  if (days > 7)                   return 'bg-amber-50 dark:bg-amber-950/30 border-amber-200 dark:border-amber-800'
...
export default async function KaynakTakibiWidget() {
  ...
  const hasAlert = data.some(s => s.daysSince === null || s.daysSince > 7)
