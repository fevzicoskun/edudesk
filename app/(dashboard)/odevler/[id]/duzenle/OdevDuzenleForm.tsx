...
export default function OdevDuzenleForm({
  hw,
  classes,
  sources,
  submissionCount,
  ...
  const classChanged = selectedClass !== hw.class_id

  useEffect(() => {
    if (!selectedClass || !dueDate) { setWeekLoad([]); return }
    let active = true
    setLoadingLoad(true)
    getClassWeekLoad([selectedClass], dueDate)
      ...
      <form action={formAction}>
        ...
        <div className="p-6 space-y-5">
          ...
          <div className="space-y-2">
            ...
            {classChanged && submissionCount > 0 && (
              <div className="flex items-start gap-2 px-3 py-2.5 bg-amber-50 border border-amber-200 rounded-xl text-xs text-amber-700">
                ...
                Bu ödev için {submissionCount} kayıt var. Sınıf değişirse eski kayıtlar yeni sınıfla eşleşmeyebilir.
              </div>
            )}
          </div>
