...
export default function InviteUserForm({ canAssignRoles }: { canAssignRoles: Role[] }) {
  ...
  function handleOpenChange(next: boolean) {
    if (!next) setFormKey(k => k + 1) // dialog kapanınca formu sıfırla
    setOpen(next)
  }
...
function InviteFormContent({
  ...
  const [state, action, pending] = useActionState(
    async (_: unknown, fd: FormData) => inviteUser(_, fd),
    null
  )
      ...
      <>
        ...
        <div className="space-y-3">
          <div className="flex items-center gap-2 text-emerald-600 dark:text-emerald-400">
            ...
            <p className="text-sm font-semibold">Hesap başarıyla oluşturuldu.</p>
          </div>
  ...
  return (
    <>
      <DialogHeader>
        ...
        <DialogDescription>
          Bilgileri doldurun, hesap oluşturulunca geçici şifre gösterilecek.
        </DialogDescription>
      </DialogHeader>
