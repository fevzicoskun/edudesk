...
export default function KronikSicil({ items }: { items: ChronicAbsentee[] }) {
  return (
    <section className="bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-xl p-4 mb-4">
      ...
      {items.length === 0 ? (
        <p className="text-sm text-gray-500 dark:text-slate-400">Uyarı eşiğini aşan öğrenci yok. 👍</p>
      ) : (
          ...
          {items.map(s => (
            <li key={s.studentId}>
              ...
              >
                ...
                <span className="text-xs text-gray-500 dark:text-slate-400 shrink-0">{s.className}</span>
